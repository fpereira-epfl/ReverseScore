...
from asp.yourmt3_remote import _extract_midi_bytes, transcribe_stem_yourmt3
...
def test_transcribe_stem_yourmt3_reuses_existing(tmp_path: Path) -> None:
    ...
    with mock.patch("asp.yourmt3_remote._load_gradio_client") as mock_load:
...
def test_transcribe_stem_yourmt3_calls_space_and_saves(tmp_path: Path) -> None:
    ...
    config = PipelineConfig(transcription_backend="yourmt3")
    with mock.patch(
        "asp.yourmt3_remote._load_gradio_client",
...
def test_transcribe_stem_dispatcher_routes_to_yourmt3(tmp_path: Path) -> None:
    ...
    with mock.patch(
        ...
        result = transcribe_stem(audio, midi_dir, config, label="piano", backend="yourmt3")
